...
class SedekahTransactionServiceTest extends TestCase
{
    ...
    public function test_creates_sedekah_and_adds_to_inventory(): void
    {
        $admin = User::factory()->admin()->create();
        $category = WasteCategory::factory()->create();

        $transaction = app(SedekahTransactionService::class)->create(
            [['waste_category_id' => $category->id, 'quantity' => 2.5]],
        ...
        $this->assertSame('2.500', (string) Inventory::firstWhere('waste_category_id', $category->id)->stock);
    ...
    public function test_does_not_create_balance_or_points(): void
    {
        ...
        app(SedekahTransactionService::class)->create(
            [['waste_category_id' => $category->id, 'quantity' => 10]],
            donor: $nasabah,
        );
    ...
    public function test_donor_must_be_nasabah_if_provided(): void
    {
        ...
        app(SedekahTransactionService::class)->create(
            [['waste_category_id' => $category->id, 'quantity' => 1]],
            donor: $owner,
        );
    }
    ...
    public function test_anonymous_donor_is_allowed(): void
    {
        ...
        $transaction = app(SedekahTransactionService::class)->create(
            [['waste_category_id' => $category->id, 'quantity' => 3]],
        );
